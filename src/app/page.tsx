...
import EventDetails from './event/[citySlug]/[eventSlug]/[eventId]/page';
...
export default function Map() {
  ...
  const addToFavorites = async (eventId: string): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/users/${auth.currentUser.uid}/addFavorite`, {
        method: 'POST',
  ...
  const removeFromFavorites = async (eventId: string): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/users/${auth.currentUser.uid}/removeFavorite`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ eventId }),
      });
              ...
              ></motion.div>
                ...
                {poiImage && (
                  <div className="popup-details_image-container">
                    <Image src={poiImage} alt={poiName} className="popup-details_image" width={300} height={200} />
