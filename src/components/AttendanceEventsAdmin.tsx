...
const AttendanceEventsAdmin: React.FC = () => {
  ...
  const handleDeleteParticipant = async (firebaseId: string) => {
    ...
    if (result.isConfirmed) {
      try {
        if (selectedEvent) {
          const response = await api.delete(`/events/${selectedEvent.eventID}/participants/${firebaseId}`);
  ...
  const handleBlockParticipant = async (firebaseId: string) => {
    ...
    if (result.isConfirmed) {
      try {
        if (selectedEvent) {
          const response = await api.put(`/events/${selectedEvent.eventID}/participants/${firebaseId}/block`);
          if (response.status === 200) {
