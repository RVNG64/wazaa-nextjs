// src/components/AttendanceEventsAdmin.tsx
...
const AttendanceEventsAdmin: React.FC = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`/users/search?term=${searchTerm}`);
  ...
  const handleSendInvitations = async (userIds: string[]) => {
    try {
      if (selectedEvent) {
        await axios.post(`/events/${selectedEvent.eventID}/sendInvitations`, { userIds });
  ...
  const handleDeleteParticipant = async (firebaseId: string) => {
    ...
    if (result.isConfirmed) {
      try {
        if (selectedEvent) {
          const response = await axios.delete(`/events/${selectedEvent.eventID}/participants/${firebaseId}`);
  ...
  const handleBlockParticipant = async (firebaseId: string) => {
    ...
    if (result.isConfirmed) {
      try {
        if (selectedEvent) {
          const response = await axios.put(`/events/${selectedEvent.eventID}/participants/${firebaseId}/block`);
          if (response.status === 200) {
